function [forest,usedsamps,testsamples,usedsampleslog,comb,allleaves,unl,allleavesy,allunlabeled,mymed]=dtforest3dis(x_eval,y_eval,ntree,sampfactor,x,y,maxleafsize,maxdepth,prf,tarpref,xlab,ylab,extdistmat)
forest=cell(ntree,1);
nseries=size(x,1);
...
% tcost=sum(distmat,2);%bu tcostu kullanma!
    ...
    [ att ,val,pnode,pnodep, tree,comb,comu,comby,combtr]=findsplitdeneme16dis(x_eval,y_eval,xx,yy,pnode,pnodep,maxleafsize,maxdepth,depth,prf,tarpref,myxlab,myylab,comb,comu,comby,combtr,myextdistmat);
    %disp(itree)
 %profile off  
med=[];
for g=2:size(comu,2)
