function [att ,val,pnode,pnodep, tree,comb,comu,comby,combtr]=findsplitdeneme16(x_eval,y_eval,x,y,pnode,pnodep,maxleafsize,maxdepth,depth,prf,tarpref,myxlab,myylab,comb,comu,comby,combtr,tcost,distmat)
dbstop if error;
...
for i=1:size(m,2)%size(x, 2);
    ...
    if x_eval(m(i))%is it numeric?
        ...
        for j=1:size(thval,1)
            ...
            %[a,aa]=min(tcost(dataaboveboth,1));Bunu commentledim cunku
            %tcost problemli
            if size(Xnumabove,1)==1
                cena=Xnumabove;%tek eleman durumu
            else
                distabove=mymedoid(Xnumabove);
                sumdistabove=sum(distabove,2);
                [a,aa]=min(sumdistabove);
                cena=Xnumabove(aa,:);
            end
            if size(Xnumbelow,1)==1
                cenb=Xnumbelow;
            else
                distbelow=mymedoid(Xnumbelow);
                sumdistbelow=sum(distbelow,2);
                [b,bb]=min(sumdistbelow);
                cenb=Xnumbelow(bb,:);
            end
           
            % [idxb,cenb,summ2]=kmedoids(Xnumbelow,1);%medoid of below
            
           % [b,bb]=min(tcost(databelowboth,1));%Bunu commentledim cunku
            %tcost problemli
...
%             difmtt3=mean(difmtt2);
            ...
            mindistt=mymedoid(x_numer);
            mindist=mindistt(dataaboveboth,databelowboth');
            ...
            %            end
            finaleval=[weightedmse weightedgini -IG -overalldist -mindiss thval(j) m(i) pnode 1 pnodep depth];
            entcell=[entcell;finaleval];%%buray? duzenle
        end
...
tcostl=tcost( d_belowbth,:);
tcostr=tcost( d_abovebth,:);
[ att,val,pnode,pnodep,tree.lchild,comb,comu,comby,combtr]=findsplitdeneme16(x_eval,y_eval,x1l_,y1l_,pnode,pnodep,maxleafsize,maxdepth,depth,prf,tarpref,xlab1l,ylab1l,comb,comu,comby,combtr,tcostl,distmat);
%entcell=[entcell;entcellb];
pnode=pnode+1;
pnodep=pnodep+1;
[att, val,pnode,pnodep, tree.rchild,comb,comu,comby,combtr]=findsplitdeneme16(x_eval,y_eval,x1r_,y1r_,pnode,pnodep,maxleafsize,maxdepth,depth,prf,tarpref,xlab1r,ylab1r,comb,comu,comby,combtr,tcostr,distmat);
