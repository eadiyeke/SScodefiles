function [performancematAVG,performancematCAT]=performeval3inducT(forest,ntree,x,testsamples,y,ytrains,settest,y_eval,x_eval,cattrg,allleavesy,xlblused,ylblused,xIT,yIT)%ytrains=ytrainset{1,j}
...
for g=1:size(y,2)
    ...
    realizedavgs=[realizedavgs,yIT(:,g)];%[realizedavgs,y(settest,g)];%1st tree's test set(arbitrary)
    %end
    else
        %for gg=1:size(testsamples,1)
    realizedcats=[realizedcats,yIT(:,g)];%[realizedcats,y(settest,g)];%1st tree's test set
    %end
    end
end
