function[performancematAVG1,readyAVGExt,readyAVGExt2,readyAVGExt3,readyAVGExt4,readyAVGExt6,readyAVGExt7,readyCATS,readyCATExt,readyCATExt2,readyCATExt3,readyCATExt4,readyCATExt6,readyCATExt7]=...
    kfoldcvfulltreev2_1tekdene_distmat(x,y,nf,maxleafsize,maxdepth,ntree,sampfactor,cattrg,categories,categoriesx,sf,extdistmat)
[xtrainset,ytrainset,r,T]= cvsets(x,y,nf);
extdistmat=extdistmat(1:size(T,1),1:size(T,1));
...
for j=1:1%nf
 ...
 for t=1:size(xmatt,2)
     xmatt(:,t)=(xtrain(:,t)-mean(xtrain(:,t)))./std(xtrain(:,t));
 end
   ...
   [forest1_1,usedsamps,testsamples,usedsampleslog,comb,allleaves,unl,allleavesy, allunlabeled]=dtforest3dis(x_eval,y_eval,ntree,sampfactor,xtrain,ytrain,maxleafsize,maxdepth,3,0,xfull(r(:,j),:),yfull(r(:,j),:),extdistmat);%rankbased için 2yi 1 yap
   ...
   % [performancematAVG1,performancematCAT1]=performeval3oob(forest1_1,ntreecnt,xfull,r(:,j),yfull,ytrain,T(:,j),y_eval,x_eval,cattrg,allleavesy,xfull(r(:,j),:),yfull(r(:,j),:),xtrain,usedsampleslog(:,1:ntreecnt));
    %[performancematAVG1,performancematCAT1]=performeval3oobfeat(forest1_1,ntreecnt,xfull,r(:,j),yfull,ytrain,T(:,j),y_eval,x_eval,cattrg,allleavesy,xfull(r(:,j),:),yfull(r(:,j),:),xtrain,usedsampleslog(:,1:ntreecnt));
