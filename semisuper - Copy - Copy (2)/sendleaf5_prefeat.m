function [y1_avg, y2_type]=sendleaf5_prefeat(tree, newinstance,x,y,x_eval,y_eval,allleavesy,mymed)
...
%         end
%         [M,I]=max(mysimmat);
myy=mean(tree.predictor,1);
        y1_avg.avg=myy';%allleavesy{1,I+1};
        y2_type.type=tree.category;%next.category; 
    else
myy=mean(next.predictor,1);
        y1_avg.avg=myy';
   y2_type.type=next.category; 
    end
else
    [y1_avg,y2_type]=sendleaf5_prefeat(next, newinstance, x,y,x_eval,y_eval,allleavesy,mymed);
end
end