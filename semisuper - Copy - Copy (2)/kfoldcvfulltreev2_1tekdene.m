function[performancematAVG1,readyAVGExt,readyAVGExt2,readyAVGExt3,readyAVGExt4,readyAVGExt6,readyAVGExt7,readyCATS,readyCATExt,readyCATExt2,readyCATExt3,readyCATExt4,readyCATExt6,readyCATExt7]=...
    kfoldcvfulltreev2_1tekdene(x,y,nf,maxleafsize,maxdepth,ntree,sampfactor,cattrg,categories,categoriesx,sf)
...
for j=1:1%nf
   ...
   xlabb=xfull(r(:,j),:);
 mysets=[xlabb;xtrain];
 for t=1:size(mysets,2)
     mysets(:,t)=(mysets(:,t)-mean(mysets(:,t)))./std(mysets(:,t));
 end
 xlabb2=mysets(1:size(xlabb,1),:);
 xtrainn=mysets(size(xlabb,1)+1:end,:);

   [forest1_1,usedsamps,testsamples,usedsampleslog,comb,allleaves,unl,allleavesy, allunlabeled]=dtforest3(x_eval,y_eval,ntree,sampfactor,xtrainn,ytrain,maxleafsize,maxdepth,3,0,xlabb2,yfull(r(:,j),:));%rankbased için 2yi 1 yap
    ...
    for ntreecnt=ntree:ntree
       % [performancematAVG1,performancematCAT1]=performeval3(forest1_1,ntreecnt,xfull,r(:,j),yfull,ytrain,T(:,j),y_eval,x_eval,cattrg,allleavesy,xfull(r(:,j),:),yfull(r(:,j),:),xtrainn);
   [performancematAVG1,performancematCAT1]=performeval3oob(forest1_1,ntreecnt,xfull,r(:,j),yfull,ytrain,T(:,j),y_eval,x_eval,cattrg,allleavesy,xfull(r(:,j),:),yfull(r(:,j),:),xtrainn,usedsampleslog(:,1:ntreecnt));
