function[readyAVG,readyAVGExt,readyAVGExt2,readyAVGExt3,readyAVGExt4,readyAVGExt6,readyAVGExt7,readyCATS,readyCATExt,readyCATExt2,readyCATExt3,readyCATExt4,readyCATExt6,readyCATExt7]=...
    kfoldcvfulltreev2_1tekdeneSSinducT_distmat(x,y,nf,maxleafsize,maxdepth,ntree,sampfactor,cattrg,categories,categoriesx,sf,numlab,xIT,yIT,extdistmat)
...
%     end
   % xtrain=xtrain_;
    xtrain=x1(numlab+1:end,:);
    ytrain=y1(numlab+1:end,:);
    xfull=x1;
    yfull=y1;
    r=1:numlab;
    r=r';
    q=ones(numlab,1);
    qq=zeros(size(xtrain,1),1);
   ...
   [forest1_1,usedsamps,testsamples,usedsampleslog,comb,allleaves,unl,allleavesy, allunlabeled]=dtforest3dis(x_eval,y_eval,ntree,sampfactor,xtrain,ytrain,maxleafsize,maxdepth,3,0,xfull(r(:,j),:),yfull(r(:,j),:),extdistmat);%rankbased için 2yi 1 yap
    ...
    for ntreecnt=ntree:ntree
     %   [performancematAVG1,performancematCAT1]=performeval3(forest1_1,ntreecnt,xfull,r(:,j),yfull,ytrain,T(:,j),y_eval,x_eval,cattrg,allleavesy,xfull(r(:,j),:),yfull(r(:,j),:),xtrain);
        ...
        [performancematAVG1,performancematCAT1]=performeval3inducT(forest1_1,ntreecnt,xfull,r(:,j),yfull,ytrain,T(:,j),y_eval,x_eval,cattrg,allleavesy,xfull(r(:,j),:),yfull(r(:,j),:),xIT,yIT);
